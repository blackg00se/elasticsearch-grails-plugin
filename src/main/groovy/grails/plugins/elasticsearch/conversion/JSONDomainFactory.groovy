...
class JSONDomainFactory {
    ...
    DomainEntity getInstanceDomainClass(Object instance) {
        /*
        DomainEntity rv = null
        if (instance?.class?.name?.contains('$$')) {
            LOG.info("instance.class.name=${instance?.class?.name}")
            LOG.info("instance.id=${instance?.id}")
            instance.properties.each {
                k, v ->
                LOG.info("instance.$k=$v, type=${v.class.name}")
            }
            Class<?> clazz = org.hibernate.proxy.HibernateProxyHelper.getClassWithoutInitializingProxy(instance)
            LOG.info("clazz.name=${clazz?.name}")
            Object instance2 = instance
            LOG.info("instance2.class.name=${instance2?.class?.name}")
            rv = domainReflectionService.getDomainEntity(instance2.class)
        } else
            rv = domainReflectionService.getDomainEntity(instance.class)
        if (!rv) LOG.warn("getInstanceDomainClass returning null!")
        return rv
        */
        //domainReflectionService.getDomainEntity(org.hibernate.proxy.HibernateProxyHelper.getClassWithoutInitializingProxy(instance))
        domainReflectionService.getDomainEntity(instance)
    }
